DROP TABLE products; -- exclui a tabela --
...
> (maior), < (menor), >= (maior ou igual), <= (menor ou igual), = (igual), <> ou != (diferente)
...
--alternativa (BETWEEN é o mesmo que maior ou igual)
SELECT productname, price FROM products
...
--Listar todos os produtos comprados por joão
...
--listar todos os clientes com id par
...
SELECT productname, idclient, price FROM products
WHERE idclient = (SELECT idclient FROM clients
WHERE firstname = 'Rodrigo')
ORDER BY price DESC